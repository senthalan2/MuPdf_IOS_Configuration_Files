...
#define SCREEN_MAX_LENGTH (CUSTOM_MAX(SCREEN_WIDTH, SCREEN_HEIGHT))
...
#define IS_IPHONE_X_MAX    (IS_IPHONE && SCREEN_MAX_LENGTH == 896.0)
...
//  UIBarButtonItem *nextButton, *prevButton, *cancelButton, *searchButton, *outlineButton, *linkButton;
  ...
  UIBarButtonItem *bookMarkListButton;
...
//  UIBarButtonItem *reflowButton;
  ...
  BOOL isEnableAnnotButton,isEnableBookMarkButton,isEnableBookMarkListButton;
...
- (instancetype) initWithFilename: (NSString*)filename path:(NSString *)cstr document: (MuDocRef *)aDoc isEnableAnnot:(BOOL) isEnableAnnot isEnableBookMark:(BOOL) isEnableBookMark continuePage:(NSNumber *) continuePage bookID:(NSString*)bookID
{
  bookId = bookID;
  continuePageNumber = continuePage;
  isEnableAnnotButton = isEnableAnnot;
  isEnableBookMarkButton = isEnableBookMark;
  isEnableBookMarkListButton = isEnableBookMark;
  ...
  if (IS_IPAD) {
    utilities.xibPostfix = @"iPad";
    
     } else if (IS_IPHONE_5) {
       utilities.xibPostfix = @"retina4";
   } else if (IS_IPHONE_6) {
     utilities.xibPostfix = @"iPhone6";
   
   } else if (IS_IPHONE_6P) {
     utilities.xibPostfix = @"iPhone6s";
    
   } else if (IS_IPHONE_X || IS_IPHONE_X_MAX || IS_IPHONE_XS) {
   
     utilities.xibPostfix = @"iPhoneX";
  
   } else {
     utilities.xibPostfix = @"retina3";
     }
...
//- (UIBarButtonItem *)newResourceBasedButton:(NSString *)resource withAction:(SEL)selector {
//    UIImage *image = [[UIImage imageNamed:resource] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
//
//    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
//        UIBarButtonItem *barButtonItem = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:selector];
//        barButtonItem.tintColor = [self colorWithHexString:@"#BE4B29"];
...
//        [button addTarget:self action:selector forControlEvents:UIControlEventTouchUpInside];
//        [button sizeToFit];
//        button.tintColor = [self colorWithHexString:@"#BE4B29"];
//        buttonView = button;
//
//        return [[UIBarButtonItem alloc] initWithCustomView:buttonView];
//    }
...
- (UIBarButtonItem *)newResourceBasedButton:(NSString *)resource withAction:(SEL)selector {
    UIImage *image = [UIImage imageNamed:resource]; // Use the original image

    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        return [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:selector];
    } else {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom]; // Use UIButtonTypeCustom
        [button setImage:image forState:UIControlStateNormal];
        [button addTarget:self action:selector forControlEvents:UIControlEventTouchUpInside];
        [button sizeToFit];
...
- (void) addMainMenuButtons
{
  ...
  if(isEnableBookMarkButton){
    [array addObject:bookMarkButton];
  }
  if(isEnableBookMarkListButton){
    [array addObject:bookMarkListButton];
  }
...
- (void) loadView
{
  ...
  if(continuePageNumber != nil ){
    int pageNumber  = [continuePageNumber intValue];
    if(pageNumber > 0){
  ...
  bookMarkButton = [self newResourceBasedButton:@"ic_bookmark" withAction:@selector(addBookMark:)];
  bookMarkListButton = [self newResourceBasedButton:@"ic_list" withAction:@selector(showBookMarks:)];
  
  
  
  annotButton = [self newResourceBasedButton:@"ic_more" withAction:@selector(onAnnot:)];
  annotButton.tintColor = [UIColor greenColor];
  ...
  //  self.navigationController.navigationBar.backgroundColor = [self colorWithHexString:@"#BE4B29"];
  //  self.navigationController.toolbar.backgroundColor = [UIColor greenColor];
  //  self.navigationController.toolbar.backgroundColor = [self colorWithHexString:@"#BE4B29"];
  [view release];
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(gotoPage:) name:@"GO_TO_PAGE" object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleHideShowBookMarkListButton:) name:@"HIDE_SHOW_BOOKMARK_LIST_BUTTON" object:nil];
  if(isEnableBookMarkListButton){
    NSMutableArray *bookMarkArray = [utilities getAllBookMarks:bookId];
    if(bookMarkArray == nil || [bookMarkArray count] == 0){
      [self hideShowBookMarkListButton:YES];
    }
    else{
      [self hideShowBookMarkListButton:NO];
    }
  }
...
- (UIColor *)colorWithHexString:(NSString *)hexString {
    ...
    if ([cleanString length] == 8) {
        ...
        return [UIColor blackColor]; // Default color if hex string is invalid
    }
}
...
//  [reflowButton release]; reflowButton = nil;
  ...
  if(isEnableAnnotButton){
    ...
    [bookMarkListButton release];bookMarkListButton = nil;
...
-(void) onPageNumberChanged:(int)pageNumber{
    if(pageNumber > 0 && pageNumber <= fz_count_pages(ctx, doc)){
      @try {
        ...
        NSDictionary *myData = @{@"pageNumber" :@(pageNumber),@"totalPages":@(fz_count_pages(ctx, doc))};
        [[NSNotificationCenter defaultCenter] postNotificationName:@"PAGE_NUMBER_UPDATE" object:nil userInfo:myData];
      }
      ...
      @finally {
          // Optional block that is always executed, regardless of whether an exception was caught
...
//  linkButton.enabled = !reflowMode;
  ...
  if(isEnableBookMarkButton){
    bookMarkButton.enabled = !reflowMode;
    bookMarkListButton.enabled = !reflowMode;
  }
...
- (void) addBookMark: (id)sender {
  utilities = [Utilities getInstance];
  if (![utilities isAlreadyBookMarked:bookId :(current + 1)]) {
    /*bookMarkAlert = [[UIAlertView alloc]
             initWithTitle: @"Add BookMark"
             message: [NSString stringWithFormat: @"Page Index \t: Page %d\nPlease enter bookMark Name", (current + 1)]
             delegate: self
             cancelButtonTitle: @"Cancel"
             otherButtonTitles: @"Add", nil];
    [bookMarkAlert setAlertViewStyle: UIAlertViewStylePlainTextInput];
    [bookMarkAlert show];*/
    
    UIAlertController *alertText = [UIAlertController
                    alertControllerWithTitle:@"Add BookMark"
                    message:[NSString stringWithFormat: @"Page Index \t: Page %d\nPlease enter bookMark Name", (current + 1)]
                    preferredStyle:UIAlertControllerStyleAlert];
    
    [alertText addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
      textField.placeholder = @"Please enter BookMark Description";
    }];
    ...
    UIAlertAction *addButton = [UIAlertAction
                  ...
                  handler:^(UIAlertAction * action) {
                    ...
                    NSString *bmDescription = alertText.textFields.firstObject.text;
                    
                    [utilities saveBookMark :bookId :(current + 1) :bmDescription];
                    [utilities showToastWithMessage :NSLocalizedString(@"Bookmark successfully added", @"") inView:self.view];
                    NSMutableArray *bookMarkArray = [utilities getAllBookMarks:bookId];
                    if(bookMarkArray == nil || [bookMarkArray count] == 0){
                      [self hideShowBookMarkListButton:YES];
                    }
    ...
    [alertText addAction:yesButton];
    [alertText addAction:addButton];
    ...
    [utilities showToastWithMessage :NSLocalizedString(@"This page is already bookmarked", @"") inView:self.view];
...
- (void) showBookMarks: (id)sender {
  NSLog(utilities.xibPostfix,@"TESTTTT");
  
  utilities = [Utilities getInstance];

  _bookMarkViewController = [[BookMarkViewController alloc] initWithNibName:[NSString stringWithFormat:@"BookMarkViewController_%@", utilities.xibPostfix] bundle:nil];
  
  _bookMarkViewController.bookId = bookId;
  
  _bookMarkViewController.view.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
  
  //[[self navigationController] pushViewController: self.bookMarkViewController animated: YES];
  [[self navigationController].view addSubview: self.bookMarkViewController.view];
  

//  if (_bookMarkViewController) {
//    navigatorThis.viewControllers = @[_bookMarkViewController];
//    windowThis.rootViewController = _bookMarkViewController;
////    [_bookMarkViewController setWindowNavigationToDismiss:windowThis navigator:navigatorThis];
//    [windowThis makeKeyAndVisible];
//    [_bookMarkViewController release];
//  }
...
- (void) onBack: (id)sender
{
  ...
  else
  {
    ...
    [windowThis release];
    [navigatorThis release];
    NSDictionary *myData = @{@"pageNumber" :@(current +1),@"totalPages":@(fz_count_pages(ctx, doc))};
    [[NSNotificationCenter defaultCenter] postNotificationName:@"CLOSE" object:nil userInfo:myData];
...
- (void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  if ([CloseAlertMessage isEqualToString:alertView.message])
  {
    if (buttonIndex == 1){
      ...
      NSDictionary *myData = @{@"pageNumber" :@(current +1),@"filePath":@(_filePath.UTF8String), @"totalPages":@(fz_count_pages(ctx, doc))};
      [[NSNotificationCenter defaultCenter] postNotificationName:@"SAVE_CLOSE" object:nil userInfo:myData];
    }
    else{
      NSDictionary *myData = @{@"pageNumber" :@(current +1),@"totalPages":@(fz_count_pages(ctx, doc))};
      [[NSNotificationCenter defaultCenter] postNotificationName:@"CLOSE" object:nil userInfo:myData];
...
- (void) showSearchResults: (int)count forPage: (int)number
{
  ...
  [self onPageNumberChanged: number+1];
  for (UIView<MuPageView> *view in canvas.subviews)
...
- (void) onTap: (UITapGestureRecognizer*)sender
{
  ...
  for (UIView<MuPageView> *view in canvas.subviews)
  {
    ...
    if (CGRectContainsPoint(view.bounds, pp))
    {
      ...
      [result switchCaseInternal:^(MuTapResultInternalLink *link) {
        ...
        [self onPageNumberChanged: link.pageNumber+1];
        tapHandled = YES;
  ...
  if (tapHandled) {
    ...
    [self onPageNumberChanged: current-2];
  } else if (p.x > x1) {
    [self gotoPage: current+1 animated: YES];
    [self onPageNumberChanged: current+2];
  } else {
...
- (void) scrollViewDidScroll: (UIScrollView*)scrollview
{
  ...
  indicator.text = [NSString stringWithFormat: @" %d of %d ", current+1, fz_count_pages(ctx, doc)];
  [self onPageNumberChanged: current+1];
  slider.value = current;
...
- (void)handleHideShowBookMarkListButton:(NSNotification *) notification {
 
  NSDictionary *showHideData = notification.userInfo;
  BOOL isHide = [[showHideData objectForKey:@"isHide"] boolValue];
  [self hideShowBookMarkListButton:isHide];
  
}

- (void)hideShowBookMarkListButton:(BOOL) isHide {
  isEnableBookMarkListButton = isHide;
  NSMutableArray *rightBarButtonItems = [self.navigationItem.rightBarButtonItems mutableCopy];
  if(isHide){
       [rightBarButtonItems removeObject:bookMarkListButton];
       self.navigationItem.rightBarButtonItems = rightBarButtonItems;
  }
  else{
       if (![rightBarButtonItems containsObject:bookMarkListButton]) {
           [rightBarButtonItems addObject:bookMarkListButton];
           self.navigationItem.rightBarButtonItems = rightBarButtonItems;
       }
  }
}
...
- (void) gotoPage :(NSNotification *) notification {
  ...
  NSString *pageNumber = [pageInfo objectForKey:@"page_number"];
  
  [self gotoPage: ((int) [pageNumber integerValue] - 1) animated:YES];
}
